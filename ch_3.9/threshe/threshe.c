...
#define NHIST 256   
#define scale 16         /* no. bins in histogram */
#define MAX 1<<scale
...
/* find threshold */
  for (i = 0, Hn = 0.0; i < NHIST; i++){
    if (prob[i] != 0.0){
      Hn -= prob[i] * (log (prob[i])-scale*log(2));
      // printf("%d\t",Hn);
    }
}
  for (i = 1, psiMax = 0.0; i < NHIST; i++) {
    for (j = 0, Ps = Hs = 0.0; j < i; j++) {
      Ps += prob[j];
      if (prob[j] > 0.0)
        Hs -= prob[j] * (log (prob[j])-scale*log(2));
          }
          // printf("%d\t",Hs);
    if (Ps > 0 && Ps < MAX){
      psi = log (Ps - ((Ps * Ps)>>scale)) + (Hs<<scale / Ps) + ((Hn - Hs)<<scale )/ (MAX - Ps);
      printf("%d\t%d\n",psi,i);
    }
    if (psi > psiMax) {
